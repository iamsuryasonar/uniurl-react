        ...
        <div style={urlsinfo?.theme?.pagecontainer} className={`${!urlsinfo?.theme ? `max-w-xl min-h-svh bg-slate-100 m-auto flex flex-col p-4 gap-4` : ''}`}>
            {urlsinfo ?
                <div className='w-full relative '>
                    {/* sticky nav */}
                    <div className={`sticky top-4 left-4 right-4  rounded-full ${isSticky ? 'backdrop-blur-sm bg-white/30 flex flex-row justify-between items-center' : 'grid justify-items-end '}`}>
                        {isSticky &&
                            <>
                        ...
                        <div onClick={() => setMenu(!menu)} className='bg-slate-200 rounded-full w-10 h-10 m-1 grid place-content-center'><FontAwesomeIcon icon="fas fa-ellipsis" /></div>
                    </div>
                    {<Transition in={menu} timeout={100}>
                        {(state) => (
                            <div className={`fixed sm:top-0 right-0 bottom-0 left-0  rounded-xl flex justify-center items-center transition-transform transform ease-in-out duration-700 ${state === 'entered' ? 'translate-y-0 ' : 'translate-y-full '}`}>
                                <div className='bg-white max-w-xl w-full sm:w-auto sm:h-5/6 md:m-20  rounded-t-lg sm:rounded-xl  p-6 flex flex-col gap-4'>
                                    ...
                                    <div className='w-full flex justify-between gap-5 md:gap-2'>
                                        <button className='w-full bg-black text-white py-1 rounded-full'>Sign up free</button>
                                        <button className='w-full bg-white text-black py-1 border  rounded-full'>Find out mode</button>
                                    </div>
                                </div>
                            </div>
                        )}
